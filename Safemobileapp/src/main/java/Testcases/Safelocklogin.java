...
public class Safelocklogin extends BaseSetup {
	...
	public void VERIFY_THAT_USER_NAVIAGETED_INTO_GET_OTP_SCREEN_SUCCESSFULLY() throws InterruptedException {
		...
		// Check if the 'Deny' button is present and click if visible
		 try {
		        WebElement denyButton = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//android.widget.Button[@resource-id='com.google.android.gms:id/negative_button']")));
		        Clickfunction(denyButton);
		        System.out.println("Deny button clicked");
		    } catch (Exception e) {
		        System.out.println("Deny button not present");
		    }
